...
namespace Nop.Services.Gdpr
{
    ...
    public partial class GdprService : IGdprService
    {
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            var customerRepository = Nop.Core.Infrastructure.EngineContext.Current.Resolve<IRepository<Customer>>();
            int storeId = await storeMappingService.CurrentStore();
            #endregion
            return await _gdprLogRepository.GetAllPagedAsync(query =>
            {
                ...
                if (storeId > 0)
                {
                    ...
                    var queryCustomer = from c in customerRepository.Table
                    ...
                    query = from c in query
                            join a in queryCustomer on c.CustomerId equals a.Id
                            select c;
                }
                #endregion
                if (customerId > 0) 
        ...
        public virtual async Task PermanentDeleteCustomerAsync(Customer customer)
        {
            ...
            var allStores = await _storeService.GetAllStoresByEntityNameAsync((await workContext.GetCurrentCustomerAsync()).Id, "Stores");
